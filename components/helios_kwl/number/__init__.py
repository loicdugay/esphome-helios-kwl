"""number — avec RawExpression fix"""
import esphome.codegen as cg
import esphome.config_validation as cv
from esphome.components import number
from esphome.const import (CONF_MAX_VALUE, CONF_MIN_VALUE, CONF_STEP,
                           ENTITY_CATEGORY_CONFIG, UNIT_CELSIUS, UNIT_PERCENT, UNIT_MINUTE)
...
def _ns(mn, mx, st, u, ic):
    return number.number_schema(HeliosKwlNumber, entity_category=ENTITY_CATEGORY_CONFIG,
        unit_of_measurement=u, icon=ic).extend({
...
async def to_code(config):
    ...
    for ck, ps, nsm, cm in _MAP:
        ...
        num = await number.new_number(config[ck],
            min_value=config[ck][CONF_MIN_VALUE],
            max_value=config[ck][CONF_MAX_VALUE],
            step=config[ck][CONF_STEP])
