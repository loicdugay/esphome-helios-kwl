"""
Sous-plateforme number — Helios KWL EC 300 Pro — Phase 2
"""
...
from esphome.const import (
    CONF_MAX_VALUE, CONF_MIN_VALUE, CONF_STEP,
...
def _number_schema(min_val, max_val, step, unit, icon):
    return number.number_schema(
        HeliosKwlNumber, entity_category=ENTITY_CATEGORY_CONFIG,
...
async def to_code(config):
    ...
    for conf_key, parent_setter, number_setter_method, control_method in _MAPPING:
        ...
        method_ptr = cg.RawExpression(f"&helios_kwl::HeliosKwlComponent::{control_method}")
