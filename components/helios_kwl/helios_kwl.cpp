...
bool HeliosKwlComponent::write_register(uint8_t reg, uint8_t value) {
  ...
  // 5. Vérification post-écriture avec délai suffisant
  // Le bus reste occupé ~20ms après les 3 messages TX (6 octets × 3 à 9600 baud)
  // Sans ce délai le poll arrive pendant que le bus traite encore → timeout → 0xFF
  delay(30);
  auto read_back = poll_register(reg, 1);
  if (!read_back.has_value() || *read_back != value) {
    uint8_t lu = read_back.has_value() ? *read_back : 0xFF;
    // Retry avec délai plus long (VMC peut être temporairement occupée)
    delay(100);
    auto retry = poll_register(reg, 1);
    if (retry.has_value() && *retry == value) {
      register_cache_[reg] = {value, millis(), true};
      ESP_LOGD(TAG, "Écriture reg=0x%02X confirmée au retry", reg);
      return true;
    }
    uint8_t lu2 = retry.has_value() ? *retry : 0xFF;
    ESP_LOGW(TAG, "Write 0x%02X = 0x%02X verif KO (lu=0x%02X) — retry 1", reg, value, lu2);
    ESP_LOGE(TAG, "Write 0x%02X echec apres retry", reg);
    // Cache optimiste quand même : l'UI reste cohérente, la VMC dira la vraie valeur
    register_cache_[reg] = {value, millis(), true};
    return false;
  }
  register_cache_[reg] = {value, millis(), true};
  ESP_LOGD(TAG, "Écriture reg=0x%02X = 0x%02X confirmée", reg, value);
  return true;
}
...
void HeliosKwlFan::control(const fan::FanCall &call) {
  ...
  // Garde anti-boucle : si rien ne change réellement, pas d'écriture RS485.
  // Évite la boucle : broadcast VMC → publish_fan_speed → sensor.on_value
  // → fan.turn_on(même vitesse) → write_register → VMC rebroadcast → ...
  bool state_changed = (call.get_state().has_value() && on != this->state);
  bool speed_changed = (speed.has_value() && (int)(*speed) != (int)(this->speed));

  if (!state_changed && !speed_changed) {
    // Rien ne change — juste rafraîchir l'UI sans écrire sur le bus
    this->publish_state();
    return;
  }
