...
CONFIG_SCHEMA = cv.Schema({
    ...
    cv.Optional(CONF_BOOST_AIRFLOW): button.button_schema(HeliosKwlBoostAirflowButton, entity_category=ENTITY_CATEGORY_CONFIG, icon="mdi:weather-windy"),
    cv.Optional(CONF_BOOST_FIREPLACE): button.button_schema(HeliosKwlBoostFireplaceButton, entity_category=ENTITY_CATEGORY_CONFIG, icon="mdi:fireplace"),
    cv.Optional(CONF_STOP_BOOST): button.button_schema(HeliosKwlStopBoostButton, entity_category=ENTITY_CATEGORY_CONFIG, icon="mdi:stop-circle-outline"),
    cv.Optional(CONF_ACKNOWLEDGE_MAINTENANCE): button.button_schema(HeliosKwlAckMaintenanceButton, entity_category=ENTITY_CATEGORY_CONFIG, icon="mdi:air-filter"),
})

async def to_code(config):
    parent = await cg.get_variable(config[CONF_HELIOS_KWL_ID])
    for key in [CONF_BOOST_AIRFLOW, CONF_BOOST_FIREPLACE, CONF_STOP_BOOST, CONF_ACKNOWLEDGE_MAINTENANCE]:
        if key in config:
