...
class HeliosKwlBoostAirflowButton : public HeliosKwlButtonBase {
 protected: void press_action() override { if (parent_) parent_->trigger_boost_airflow(); }
};
class HeliosKwlBoostFireplaceButton : public HeliosKwlButtonBase {
 protected: void press_action() override { if (parent_) parent_->trigger_boost_fireplace(); }
};
class HeliosKwlStopBoostButton : public HeliosKwlButtonBase {
 protected: void press_action() override { if (parent_) parent_->stop_boost_cycle(); }
};
class HeliosKwlAckMaintenanceButton : public HeliosKwlButtonBase {
 protected: void press_action() override { if (parent_) parent_->acknowledge_maintenance(); }
};
} }
